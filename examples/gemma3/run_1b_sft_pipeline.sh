...
usage() {
    cat <<'EOF'
Usage:
  ./examples/gemma3/run_1b_sft_pipeline.sh --stage <prepare|preflight|overfit|smoke|launch|export|evaluate|full> [options]

Key options:
  --hf-token <token>                HuggingFace token. Falls back to $HF_TOKEN.
  --hf-model <model>                Base pretrained HF model. Default: google/gemma-3-1b-pt
  --hf-config-ref <model>           HF config/tokenizer ref for export/eval. Default: google/gemma-3-1b-it
...
HF_CONFIG_REF="google/gemma-3-1b-it"
...
OVERFIT_PACK_PATH=""
REASONING_EVAL_PATH=""
BASE_CHECKPOINT="/home/jovyan/models/gemma3-1b-mcore-sft-base"
